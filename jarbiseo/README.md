...
- **ElevenLabs 고품질 음성** (Phase 2 목표 #2): `.env`에 `ELEVENLABS_API_KEY`를 넣으면 음성 선택기에 4가지가 나타납니다 — **엘리시아**(계정에서 이름으로 자동 검색) + 여성 3종(사라·릴리·아리아). 선택은 기억되며, ElevenLabs 실패 시 브라우저 음성으로 자동 폴백됩니다. 키는 시작 스크립트가 다음 실행 때 한 번 물어봅니다(Enter로 건너뛰기 가능)
